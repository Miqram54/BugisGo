...
$query = "
    SELECT 
        ...
        ulasan.namawahana AS wahanaName, 
        ulasan.rating, 
        ulasan.komentar, 
        ulasan.tanggalUlasan 
    FROM ulasan
    JOIN users ON ulasan.userID = users.userID
    ORDER BY ulasan.tanggalUlasan DESC
";
                        ...
                        <?php if ($result->num_rows > 0): ?>
                            <?php $no = 1; while ($row = $result->fetch_assoc()): ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($row['wahanaName']); ?></td>
                                    <td>
                                        <?php 
                                            // Menampilkan bintang berdasarkan rating
                                            echo str_repeat("★", (int)$row['rating']) . str_repeat("☆", 5 - (int)$row['rating']);
                                        ?>
                                    </td>
                                    <td><?php echo htmlspecialchars($row['komentar']); ?></td>
                                    <td><?php echo date('d M Y H:i', strtotime($row['tanggalUlasan'])); ?></td>
                                </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="6" class="text-center">Tidak ada ulasan yang tersedia.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
